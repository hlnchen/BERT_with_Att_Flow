...
class BERT_plus_BiDAF(nn.Module):
    def __init__(self, question_len = 62, if_extra_modeling = False, if_bidirectional = False, if_attention_map = False):
        """
        ...
        self.attention_layer = AttFlow(feature_dimension=self.hidden_dim, if_attention_map = self.if_attention_map)
        
        # Additional modeling layer LSTM/Transformer:
        if if_extra_modeling:
            self.modeling_layer = nn.LSTM(input_size=4*self.hidden_dim,hidden_size=2*self.hidden_dim, num_layers=2, batch_first = True, bidirectional= if_bidirectional)
        else:
            self.modeling_layer = None
            
        # Prediction
        if self.modeling_layer:
            if self.modeling_layer.bidirectional == True:
    ...
    def forward(self, input_ids, input_mask, start_pos = None, end_pos = None):
        ...
        bert_context_features = torch.cat((bert_features[:, 0, :].unsqueeze(dim=1), bert_features[:, self.question_len+1:, :]), dim=1) # (N,T,d), T=450 NOTE: T+J = 512， context is [CLS][context][SEP]

        # Concatenate and feed into attention
        if self.if_attention_map:
            c2q_attention, q2c_attention, weight_c2q, weight_q2c =  self.attention_layer(bert_context_features, bert_question_features)
        else:
        ...
        if self.modeling_layer:
            combined_features = self.modeling_layer(combined_features)[0] #(N,T,2d), or (N,T,4d) if bidirectional
        ...
        if self.if_attention_map:
            return total_loss, start_logits, end_logits, weight_c2q, weight_q2c
        else:
